...
for s in system_strs:
    ...
    for exp in list(expt_and_obs_for_system[s].keys()):
        for obs in list(expt_and_obs_for_system[s][exp].keys()):
            ...
            # the number of bins used for calibration for each observable (files may contain more bins)
            ...
            for idf in range(number_of_models_per_run):
                ...
                if run_closure==True:
                    ...
                    pseudo = np.array(dsv[s][closure_val_pt][obs]['mean'])
...
#print(Y_exp_data['Au-Au-200']['dNch_deta_cen_10_20']['mean'])
